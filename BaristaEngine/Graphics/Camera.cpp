...
void Camera::LookAt(XMFLOAT3 lookAtPos)
{
	...
	auto distance = sqrt((lookAtPos.x * lookAtPos.x) + (lookAtPos.z * lookAtPos.z));
	auto pitchAngle = atan(this->pos.y / distance);

	auto yawAngle = 0.0f;
	if (lookAtPos.x != 0.0f) {
		yawAngle = atan(lookAtPos.x / lookAtPos.z);
	}
	if (lookAtPos.z > 0)
		yawAngle += XM_PI;
